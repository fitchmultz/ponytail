...
//   1. Writes the active mode for this session (or the legacy flag without an ID)
...
function activate({ session_id: sessionId } = {}) {
  // Resume and compaction must reuse the chosen mode; a missing flag means off.
  const mode = process.argv[2] === '--restore' ? (readMode(sessionId) || 'off') : getDefaultMode();
...
if (isCopilot || isCursor) activate();
else withHookInput(activate);
