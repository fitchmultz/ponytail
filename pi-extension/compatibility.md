...
The extension adds one named `ponytail` prompt section. Mode changes during work
apply at the next provider request without interrupting the task or making an
extra request. Unchanged requests keep their complete previous message prefix.
...
the extension is disabled. With the extension loaded, only the cloned per-run
base-skill descriptor is hidden from automatic model invocation. Explicit skill
use does not change persistent mode; use `/ponytail` for that. No duplicate
...
`npm test --prefix pi-extension`. The native tests use Pi's resource loader, session runtime,
package filters, and deterministic faux provider. They make no live model calls.
