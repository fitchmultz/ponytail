...
Ponytail is an agent-portable skill distribution. `hooks/ponytail-core.md` and
`hooks/ponytail-modes.json` define the shared policy; workflow bodies live in
`skills/`. The generator produces self-contained skills, rules, and command
wrappers. Host adapters load those resources without separate policy copies.
...
| Grok Build | root `plugin.json`, `.grok-plugin/marketplace.json`, `skills/`, `commands/` | `grok plugin install fitchmultz/ponytail --trust`, then enable. Grok can auto-invoke ponytail from its coding-task skill description; `/ponytail` makes activation explicit. Grok lifecycle hooks are not used because passive hook output cannot inject instructions. |
...
| pi | `pi-extension/`, `skills/`, `hooks/` | Pi 0.87.0+: one named policy section, branch-local mode controls effective at the next model call, and native skill aliases. Standalone skills remain available without the extension. See the README for full-prompt replacement and `off` boundaries. |
...
- `skills/ponytail-gain/SKILL.md`: historical benchmark evidence and limitations
